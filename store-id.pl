...
# ============================================================= #
...
} elsif ($u=~ m/^https?\:\/\/.*redirector\.googlevideo\.com.*videoplayback.*/){
        @v = m/[&?]v\=([^\&\s]*)/;
        @cpn = m/[&?]cpn\=([^\&\s]*)/;
        unless (-e "/tmp/@cpn"){
        open FILE, ">/tmp/@cpn";
	...
	$out="OK store-id=http://bandi-tech.squid.internal/id=@id@mime@range";
...
# ============================================================= #
    ...
    $out="OK store-id=http://bandi-tech.squid.internal/" . $2  . "." . $3 ;
